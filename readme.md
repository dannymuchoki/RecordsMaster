...
You need the --context flag because .NET doesn't read environment variables when checking the database context (i.e. it needs you to explicitly say which database to use). The SQLite and SQL contexts each have context factory classes in the 'Data' directory that run at design time. This solves a type issue that kept coming up when I just had two different AppDbContexts.
...
# App overall
The app runs on three models and the default ApplicationUser.
...
2. Three PDF printing services:
   ...
   - PDFPrintService is cross-platform and compatible with .NET 9 and .NET 10.
...
1. The two AppDbContexts. One for SQLite and the other for Production SQL. That's why there are two different ways to run migrations for Development and Production, and why you need the --context flag.
...
# Coming changes
.NET 9 reaches EOL in November 2026. The app defaults to .NET 10 in RecordsMaster.csproj. To run in .NET 9, remove '.old' from the .Net9.csproj file. Then:
