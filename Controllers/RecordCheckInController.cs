...
namespace RecordsMaster.Controllers
{
    ...
    /* Only admins can check in a record  */
    public class RecordCheckInController : Controller
    {
        ...
        public async Task<IActionResult> CheckIn(Guid id)
        {
            ...
            // Notify the user only after the database update has succeeded.
            await SendCheckInNotificationAsync(recordItem, checkoutHistory);

            TempData["Message"] = "Record successfully checked in.";
            return RedirectToAction("CheckOut", "RecordCheckOut", new { id });
        }

        // Sends a check-in notification email to the user who had the record checked out.
        private async Task SendCheckInNotificationAsync(RecordItemModel recordItem, CheckoutHistory? checkoutHistory)
        {
            if (checkoutHistory == null)
            {
                return;
